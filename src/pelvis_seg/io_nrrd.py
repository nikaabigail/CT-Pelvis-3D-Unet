...
def build_pairs_from_dirs(ct_dir: str, lab_dir: str, domain: str = "smir"):
    ct_dir = Path(ct_dir)
    ...
    pairs = [
        {"case": k, "image": str(ct_map[k]), "label": str(lab_map[k]), "domain": domain}
        for k in keys
    ]
    return pairs, missing_ct, missing_lab


def build_pairs_same_filename(ct_dir: str, lab_dir: str, domain: str):
    ct_dir = Path(ct_dir)
    lab_dir = Path(lab_dir)
    ...
    ct_map = {p.name: p for p in ct_files}
    lab_map = {p.name: p for p in lab_files}
    ...
    for fname in keys:
        ...
        pairs.append(
            {
                "case": case,
                "image": str(ct_map[fname]),
                "label": str(lab_map[fname]),
                "domain": domain,
            }
        )
